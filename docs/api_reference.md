...
| Field | Description | Default | Validation |
| --- | --- | --- | --- |
| `enabled` _string_ | enabled is for enabling the use of cert-manager for obtaining and renewing the<br />certificates used for webhook server, instead of built-in certificates.<br />Use `true` or `false` to indicate the preference. This field is immutable once set. | false | Enum: [true false] <br />Required: \{\} <br /> |
...
| `issuerRef` _ObjectReference_ | issuerRef contains details of the referenced object used for obtaining certificates. When<br />`issuerRef.Kind` is `Issuer`, it must exist in the `.spec.controllerConfig.namespace`.<br />This field is immutable once set. |  | Optional: \{\} <br /> |
...
| `labels` _object (keys:string, values:string)_ | labels to apply to all resources created for the external-secrets operand deployment. |  | MaxProperties: 20 <br />MinProperties: 0 <br />Optional: \{\} <br /> |
...
| `controllerConfig` _[ControllerConfig](#controllerconfig)_ | controllerConfig is for specifying the configurations for the controller to use while installing<br />the `external-secrets` operand. |  | Optional: \{\} <br /> |
...
| `enabled` _string_ | enabled indicates whether the feature is active. |  | Enum: [true false] <br />Required: \{\} <br /> |
...
| Field | Description | Default | Validation |
| --- | --- | --- | --- |
| `httpProxy` _string_ | httpProxy is the URL of the proxy for HTTP requests. |  | MaxLength: 2048 <br />MinLength: 0 <br />Optional: \{\} <br /> |
| `httpsProxy` _string_ | httpsProxy is the URL of the proxy for HTTPS requests. |  | MaxLength: 2048 <br />MinLength: 0 <br />Optional: \{\} <br /> |
| `noProxy` _string_ | noProxy is a comma-separated list of hostnames and/or CIDRs and/or IPs for which the proxy should not be used. |  | MaxLength: 4096 <br />MinLength: 0 <br />Optional: \{\} <br /> |
